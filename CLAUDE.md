...
**Audio pipeline** (`main.rs`, `db.rs`): CPAL input stream calls back with audio buffers. `compute_loudness()` in `db.rs` is the single source of truth for the RMS/peak/hybrid/dB calculation. The callback must stay non-blocking — alert playback and notifications are spawned on separate threads.
...
- `compute_loudness()` in `db.rs` must be the single implementation — don't duplicate the math elsewhere.
